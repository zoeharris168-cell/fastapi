...
def test_user(client):
    user_data = {"email": "testcreateuser@gmail.com", "password": "password123"}
...
def test_user1(client):
    user_data = {"email": "testcreateuser1@gmail.com", "password": "password123"}
    res = client.post('/users/', json=user_data)
    assert res.status_code == 201
    new_user = res.json()
    new_user['password'] = user_data['password']
...
def test_posts(test_user, session, test_user1):
    ...
    session.add_all([models.Post(**d) for d in posts_data])
    session.commit()
